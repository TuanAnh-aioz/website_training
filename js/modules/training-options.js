...
export class TrainingOptions {
  ...
  initElements() {
    ...
    const initInterval = setInterval(() => {
      ...
      this.elements.addTransformBtn.addEventListener("click", () =>
        this.addTransform()
      );
  ...
  async fetchPlatforms() {
    try {
      ...
      if (select)
        select.innerHTML =
          '<option value="" disabled>Error loading platforms</option>';
    }
  }
  ...
  renderPlatforms() {
    const container =
      this.elements.platformContainer ||
    ...
    if (!this.selectedPlatforms || this.selectedPlatforms.length === 0) {
      ...
      this.elements.platformPreview &&
        (this.elements.platformPreview.textContent = "");
      return;
    }
    ...
    this.selectedPlatforms.forEach(({ key, info }) => {
      ...
      ramRow.innerHTML = `<span>RAM:</span><span>${(
        info.system.system_ram / 1e9
      ).toFixed(1)} GB</span>`;
      ...
      gpuList.innerHTML =
        info.system.gpu_devices && info.system.gpu_devices.length
    ...
    if (this.elements.platformPreview) {
      this.elements.platformPreview.textContent = JSON.stringify(
        this.selectedPlatforms.map((p) => ({
          platform: p.info.system.os,
  ...
  addSingleItem(type) {
    ...
    const available =
      type === "optimizer" ? AVAILABLE_OPTIMIZERS : AVAILABLE_SCHEDULERS;
    ...
    const placeholderText =
      type === "optimizer" ? "Replace optimizer" : "Replace scheduler";
  ...
  renderItems(containerKey, items, available) {
    ...
    items.forEach((item) => {
      ...
      btnContainer.className = `${containerKey.replace(
        "Container",
        ""
      )}-buttons`;
      ...
      btnSettings.addEventListener("click", () => {
        paramsPanel.style.display =
          paramsPanel.style.display === "none" ? "block" : "none";
      });
  ...
  populateTransforms() {
    this.elements.transformSelect.innerHTML =
  ...
  getCurrentTransforms() {
    return this.currentMode === "train"
      ? this.transformsData.train
      : this.transformsData.val;
  }
  ...
  renderTransforms(refresh = true) {
    ...
    if (!transforms.length) {
      this.elements.transformContainer.innerHTML =
    ...
    transforms.forEach((t) => {
      ...
      header
        ...
        .addEventListener("click", () => this.removeTransform(t.id));
      header
        .querySelector(".btn-settings")
        .addEventListener("click", () => this.toggleParamPanel(t.id));
      div.appendChild(header);
